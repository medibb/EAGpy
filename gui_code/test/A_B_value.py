...
class MyApp(QWidget):
    ...
    def draw_lowpass_plot(self, ch, idx):
        ...
        if self.setting_time:
            ...
            A_value_data = y[startTime-12 : startTime+12]
            B_value_data = y[endTime-12 : endTime+12]
            max_data = round(np.mean(A_value_data), 3)
            min_data = round(np.mean(B_value_data), 3)
